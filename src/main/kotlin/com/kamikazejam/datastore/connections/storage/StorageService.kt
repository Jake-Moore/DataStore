...
abstract class StorageService : LoggerService(), Service {
     ...
     */
    abstract suspend fun <K : Any, X : Store<X, K>> save(collection: Collection<K, X>, store: X): Boolean
     ...
     */
    abstract suspend fun <K : Any, X : Store<X, K>> get(collection: Collection<K, X>, key: K): X?

    /**
     * @return How many Stores are stored in a collection within this store.
     */
    abstract suspend fun <K : Any, X : Store<X, K>> size(collection: Collection<K, X>): Long

    /**
     * Check if a Store is stored in a given collection.
     */
    abstract suspend fun <K : Any, X : Store<X, K>> has(collection: Collection<K, X>, key: K): Boolean

    /**
     * Remove a Store from a given collection.
     */
    abstract suspend fun <K : Any, X : Store<X, K>> remove(collection: Collection<K, X>, key: K): Boolean

    /**
     * Remove ALL Stores from a given collection.
     * @return The number of Stores removed.
     */
    abstract suspend fun <K : Any, X : Store<X, K>> removeAll(collection: Collection<K, X>): Long

    /**
     * Retrieve all Stores from a specific collection.
     */
    abstract suspend fun <K : Any, X : Store<X, K>> getAll(collection: Collection<K, X>): Flow<X>

    /**
     * Retrieve all Store keys from a specific collection.
     */
    abstract suspend fun <K : Any, X : Store<X, K>> getKeys(collection: Collection<K, X>): Flow<K>
